  ...
  "The catalog-client" should {
    "call catalog-manager/v1/dataset-catalogs return a non empty list" in
      new WithServer(app = application, port = 9000) {
        ...
        val catalogManager = new Catalog_managerClient(clientManager)("***REMOVED***://localhost:9001")
        //val test: List[it.gov.***REMOVED***.catalogmanager.MetaCatalog] = Await.result(catalogManager.datasetcatalogs("test:test"), Duration.Inf)
