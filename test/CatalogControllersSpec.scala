...
import it.gov.***REMOVED***.catalogmanager
import it.gov.***REMOVED***.catalogmanager.client.Catalog_managerClient
  ...
  "The catalog-manager" should {
      ...
      new WithServer(app = application, port = 9000) {
        WsTestClient.withClient { implicit client =>
          val response: WSResponse = Await.result[WSResponse](client.
            url(s"***REMOVED***://localhost:9001/catalog-manager/v1/dataset-catalogs").
    ...
    "The catalog-manager" should {
      "Call catalog-manager/v1/dataset-catalogs/{logical_uri} return ok status" in
        new WithServer(app = application, port = 9000) {
          val logicalUri = "***REMOVED***://dataset/std/standard/standard/uri_cultura/standard"
          val url = s"***REMOVED***://localhost:9001/catalog-manager/v1/dataset-catalogs/$logicalUri"
          println(url)
          WsTestClient.withClient { implicit client =>
            val response: WSResponse = Await.result[WSResponse](client.
              url(url).
