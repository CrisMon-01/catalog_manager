package it.gov.***REMOVED***.catalogmanager.utilities
...
object ConfigReader {
  ...
  def localUrl :String = config.localUrl.getOrElse("***REMOVED***://localhost:9001")
  def securityManHost :String = config.securityManHost.getOrElse("***REMOVED***://localhost:9002/security-manager")
  def cookieExpiration:Long = config.cookieExpiration.getOrElse(30L)// 30 min by default
  def ingestionUrl :String = config.ingestionUrl.getOrElse("***REMOVED***://localhost:9003")
