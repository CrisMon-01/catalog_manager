package it.gov.***REMOVED***.catalogmanager.kylo
 ...
 // val inferJson = Json.parse(inferKylo)
  ...
  def buildUserProperties(userProperties :Seq[JsValue], metaCatalog: MetaCatalog, fileType :String) :JsArray = {
    ...
    val result = userProps.map(x => {
      ...
      systemName match {
        case "***REMOVED***_type" => { if (metaCatalog.operational.is_std)
                                x + ("value" -> JsString("standard"))
                              else
                                x + ("value" -> JsString("ordinary"))
                            }
        case "***REMOVED***_domain" => x + ("value" -> JsString(metaCatalog.operational.theme))
        case "***REMOVED***_subdomain" => x + ("value" -> JsString(metaCatalog.operational.subtheme))
        case "***REMOVED***_format" => x + ("value" -> JsString(fileType))
        case "***REMOVED***_opendata" => { if (metaCatalog.dcatapit.privatex.getOrElse(true))
                                   x + ("value" -> JsBoolean(false))
                                 else
                                   x + ("value" -> JsBoolean(true))
        }
        case "***REMOVED***_owner" => x + ("value" -> JsString(metaCatalog.dcatapit.author.getOrElse("")))
      }
    })
    JsArray(result)
  }
